...
const ProgramHero = ({ onEnroll }: ProgramHeroProps) => {
  ...
  const showOriginal = course ? course.originalPrice > course.currentPrice : false;

  const introParagraphs = course?.descriptionParagraphs?.length
    ? course.descriptionParagraphs.map((para) => parseBoldText(para))
    : course?.description
      ? [course.description]
      : [];
